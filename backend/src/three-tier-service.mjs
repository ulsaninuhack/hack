...
function laneItem(record, proposal, referenceDate) {
  ...
  const item = {
    ...
    lane: proposal.lane,
    assignment_status: proposal.lane === 'phone' ? 'confirmed' : proposal.status,
    ...
    async getTodayLanes({ sessionId, referenceDate, workerId }) {
      ...
      const completed = records
        ...
        .map((record) => ({
          ...
          급성도_점수: record.triage.급성도_점수 ?? null,
          취약도_점수: record.triage.취약도_점수 ?? null,
          연속_미응답: record.household.contact.consecutive_no_answer_count ?? 0,
