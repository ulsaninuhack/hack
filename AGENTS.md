...
- `backend/src/contact-triage-synthetic-scenario.mjs`: fresh-session map preview selects one deterministic `[합성 시나리오]` example per current admin dong (162 examples covering 156 geometry zones). It is display-only, does not mutate workflow, and must never be presented as observed resident data.
- `voice/`: isolated Node 24 voice-input module. Stage 3a converts consented, PII-masked text to a fixed JSON contract with OpenAI Structured Outputs. Stage 3b adds validated WAV/MP3 file transcription and immediately reuses 3a. The P3 adapter maps that output into a Critic-reviewed, confirmation-required ContactOps candidate; live calls are explicitly env-gated. Realtime remains intentionally unimplemented.
