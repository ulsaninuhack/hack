...
- Do not add names, phone numbers, unit numbers, resident attributes, or any other personal or household-level source record.
- Public residential-building addresses and representative points may anchor synthetic ContactOps tasks. Keep generic IDs (`연결단원 001`, `SYN-HH-*`), `synthetic=true`, `not_real_resident=true`, and source lineage; never claim that an address is linked to an actual resident or observed personal state.
