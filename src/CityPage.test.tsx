...
describe('CityPage (시·구 /city)', () => {
  it('renders district briefing with structure and load side by side, never case IDs (INV17)', async () => {
    ...
    expect(screen.getByLabelText('구조 맥락 비율 지표')).toBeInTheDocument()
    expect(screen.getByLabelText('구 단위 관측 수치')).toBeInTheDocument()
    expect(screen.getByLabelText('우선 조치 권고')).toBeInTheDocument()
