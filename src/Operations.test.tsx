...
describe('P2 manager flow', () => {
  ...
  it('loads recommendation records and provides the existing map plus list alternative', async () => {
    ...
    expect(screen.getByText(/왼쪽 목록/)).toBeInTheDocument()
  })
  ...
  it('shows the manager breadth API transfer queue, separated distributions, tuning warning, and 13 approved-visit hint', async () => {
    ...
    expect(screen.getAllByText('급성도').length).toBeGreaterThan(0)
    expect(screen.getAllByText('취약도').length).toBeGreaterThan(0)
  })
  ...
  it('keeps the 156-zone public context separate from synthetic contact work and shows all four unvalidated indicators', async () => {
    ...
    render(<ManagerPage />)
    expect(await screen.findByText('검증 전 모델 산출')).toBeInTheDocument()
    expect(screen.getByText('동네 구조 맥락')).toBeInTheDocument()
    expect(screen.getByText('업무 현황')).toBeInTheDocument()
    ...
    expect(screen.getByText('기준일이 서로 다른 참고값입니다 (분자 2024-12-31 · 분모 2026-06-30).')).toBeInTheDocument()
  ...
  it('opens a separately selected operations zone contribution panel from the manager map', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '업무 현황' }))
    await user.click(screen.getByRole('button', { name: '지도 구역 선택' }))
    expect(await screen.findByRole('heading', { name: '구역 업무 요약' })).toBeInTheDocument()
    ...
    expect(screen.getByText(/색은 급성도, 원 크기는 취약도/)).toBeInTheDocument()
    expect(screen.getByText('가상 예시 포함')).toBeInTheDocument()
    expect(screen.getByText('세션 기록 1건 · 가상 예시 1건 · 미기록 1건')).toBeInTheDocument()
    expect(screen.getByText(/행정동마다 1건의 고정 예시/)).toBeInTheDocument()
