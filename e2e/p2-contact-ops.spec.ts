...
test('surveyor contact raises a recommendation and only a manager can approve it', async ({ browser }) => {
  ...
  await expect(page.getByRole('option').first()).toContainText(CASE_ID)
  expect(firstBefore).not.toContain(CASE_ID)
  ...
  await page.getByLabel('결정 사유').fill('Playwright 합성 수직 흐름 승인')
