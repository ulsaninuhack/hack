...
## 2026-08-13 — /m: Gamegoo 디자인 적용 + 입력 흐름 사용자 피드백 반영

- /m을 center와 같은 Gamegoo 토큰(바이올렛 #5a42ee, 보더 카드, 10px 버튼,
  세그먼트 레인 탭)으로 재스타일. 터치 타깃 48px 게이트 준수(44px 회귀 수정).
- 입력 방법을 2중 1택으로 정리: 음성 파일 / 문답 또는 직접 체크. 문답 화면에서
  직접 체크리스트로 즉시 이동 가능.
- 문답 재답변: 이미 답한 질문을 눌러 그 지점부터 다시 답할 수 있음.
- 방문 위치 지도에 명시적 X 닫기 버튼 추가(controlled details).
- 관찰 체크리스트를 레인별로 재구성: 전화 레인은 "주변 확인 신호"(주변에서
...
## 2026-08-13 — 선택 대상자 음성 문맥과 사회적 고립 후보 매핑

- 대상자 화면에서 올린 통화 메모에 선택된 case 문맥을 함께 전달해, 대상자의 1인칭
  인용·욕설이 연결단원 컨디션으로 오분류되던 문제를 수정.
- AI가 `누워만 있음`을 `외출_없음`, 사회적 접촉 부재를 정규 `관계망 없음` 신호로
  추출하고 기존 체크리스트의 `관계망_유무=없음`으로 연결. 점수 엔진은 수정하지 않음.
- 같은 AI 정규 신호로 공과금 체납·최근 건강/마음 괴로움·관계망·연락
  빈도까지 기존 점수 항목을 채움. 매핑되지 않는 내용은 모바일 `기타 특이사항 확인`에
  표시하고, 조사원이 해당 체크리스트를 확인해야 점수에 반영됨.
- 4.3초 합성 M4A 실 OpenAI 검증: `connected_concern`, 식사 심각, 외출 없음,
  관계망 없음, 확인 필요 후보 생성. 기존 엔진 확인 결과 급성도 37 `주시`, 취약도
  62.60, 재연락 기한 단축.
- 결정론 검증: 음성 53개 통과(라이브 opt-in 1 skip), 백엔드 연결 테스트에서 같은
...
## 2026-08-13 — 실시간 통화 체크리스트 병렬 AI 갱신

- 연락 대상의 확정 발화마다 누적 전사문으로 새 Planner–Critic 요청을 즉시 시작한다.
  이전 요청을 기다리지 않으며, 현재 case·revision·call 범위의 최신 요청만 화면을
  갱신해 늦게 끝난 과거 응답이 새 후보를 덮어쓰지 못한다.
- 통화 중 체크리스트 6개를 항상 표시하고 미결정값은 `미확인`으로 유지한다. 다음
  확인 질문은 구체적인 모순 확인 뒤 아직 확인하지 않은 항목을 우선한다.
- 공과금은 기간을 추정하지 않고 `공과금 체납`으로 표시한다. AI 추출 뒤 명시적
  납부·체납 표현을 결정론 검증기로 보완하며, 혼합 문장과 `공과급` STT 오탈자도
  골든 테스트에 포함했다.
- 390×844에서 체크리스트 2열 레이아웃과 한글 줄바꿈을 브라우저로 검증했다.
  `/m` 상단 역할·동·내비게이션 설명은 제거했다.
