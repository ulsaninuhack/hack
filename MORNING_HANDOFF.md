...
| P3 `/m` | pass | 프런트 테스트 5건 + p9 골든(수동 경로) |
...
- INV17: 구 단위 집계·/city 본문에 `SYN-HH-` 부재(E2E는 body innerText로 단언).
...
- [ ] 워딩·실기기·664 튜닝 경고 등 기존 morning 게이트 전부 유지.
...
3. `/m` 390×844 — 전화 탭 → `SYN-HH-2812551000-0001` → [가상] 전화 탭(발신 모사)
