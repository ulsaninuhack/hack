...
- 골든 스파인: 시 지도(`/city` 구 브리핑·AI 요약 라벨) → 동 센터 오늘 배치 일괄
  확인(INV14: 확인 전 POST 없음) → 모바일 실시간 통화 진입점·수동 체크리스트 제출(62점
...
- INV14: 확인·제출·승인 없이 상태가 확정되는 경로 없음. Ralph가 찾은 두 구멍의
  ...
  리비전은 다시 '미확인'.
- INV15: 호환 API의 전화번호는 계속 `010-0000-XXXX`로 제한하며 실번호 패턴 재귀 grep
  게이트를 유지한다. `/m`에서는 오래된 가상 발신 모사를 노출하지 않는다.
...
3. `/m` 390×844 — 전화 탭 → `김영자 어르신`과 주소 확인 → 실시간 통화 시작
   (실기기 리허설이 없으면 직접 체크 fallback) → 최근 외출은 통화 확인, 우편물 등 5징후는
   방문·주변 확인에서 수동 입력 → 미응답+우편물 적체+식사 심각 → 확인하고 제출
